...
foreach($events as $event){
    ...
    if($event->allday==1){
        echo    "DTSTART;VALUE=DATE:".date('Ymd', strtotime($event->date))."\n";
        echo    "DTEND;VALUE=DATE:".date('Ymd', strtotime($event->date))."\n";
    }else{
        echo        "DTSTART;TZID=".$tz.":".date('Ymd', strtotime($event->date))."T".date('Hi', strtotime($event->sttime))."00\n";
        echo        "DTEND;TZID=".$tz.":".date('Ymd', strtotime($event->date))."T".date('Hi', strtotime($event->entime))."00\n";
    }
